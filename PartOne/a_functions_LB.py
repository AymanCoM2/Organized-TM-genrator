def replaceHeaderFooterQuery(replacement):
    headerFooterQuery = """ 
        ...
        FROM LB.DBO.OINV T0
        LEFT JOIN LB.DBO.NNM1 N1 ON N1.Series = T0.Series
        LEFT JOIN  LB.DBO.OUSR T00 ON T0.USERSIGN = T00.INTERNAL_K
        WHERE 
...
def replaceRowsQuery(replacement):
    rowsQuery = """ 
        ...
        FROM 
        (LB.DBO.OINV T0 inner join LB.DBO.INV1 T1 on T1.DocEntry= T0.DocEntry)
        LEFT JOIN (LB.DBO.OWHS W0 LEFT JOIN LB.DBO.OLCT L0 ON W0.Location = L0.Code)
        ON W0.WhsCode = T1.WhsCode
        WHERE T1.DocEntry IN (SELECT T0.DocEntry FROM LB.DBO.OINV T0 WHERE T0.CANCELED ='N') and 
        T1.DocEntry = 'X'
    """
